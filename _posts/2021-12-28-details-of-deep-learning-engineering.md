...
7. opencv和dataloader的神奇联动。很多人经常来问为啥要写cv2.setNumThreads(0)，其实是因为cv2在做resize等op的时候会用多进程，当torch的dataloader也是多进程的时候，多进程套多进程，很容易就卡死了（具体哪里死锁了没探究很深，之前 @wangyuzhi  有个[帖子](https://discourse.brainpp.cn/t/topic/19323/8)说过类似的坑）。除了setNumThreads之外，通常还要加一句cv2.ocl.setUseOpenCL(False)，原因是cv2使用opencl和cuda一起用的时候通常会拖慢速度，加了万事大吉，说不定还能加速。
...
3. Pytorch的SyncBN在1.5之前一直实现的有bug，所以存在使用SyncBN结果掉点的问题。现在官方已经修复了问题（**任何官方都会犯错，贵司的MegEngine团队也是一样，所以请积极report问题然后耐心一些**）。
4. 多卡为何不能向单卡一样严格复现？可复现是很多炼丹师关心的问题，很多人看了这里的[pytorch文档](https://pytorch.org/docs/stable/notes/randomness.html)之后，都会尝试复现锁死torch里面的seed再加一个deterministic，但是通常是单卡可以严格复现loss曲线，但是多卡就不会了，原因其实是多卡做parameter的reduce的时候，存在一个加操作的顺序问题。而**浮点数的加法其实不遵守交换律的**。很多非计算机软件专业的同学可能并不理解这一件事，直接自己开一个终端体验可能会更好：
    ```python3
    print(1e100 + 1e-4 + -1e100)  # ouptut: 0
    print(1e100 + -1e100 + 1e-4)  # output: 0.0001
    ```
5. 用了多卡开多尺度训练，明明尺度更小了，但是速度好像不是很理想？这个问题涉及到多卡的原理，因为分布式训练的时候，在得到新的参数之后往往需要进行一次同步。假设有两张卡，卡0的尺度非常小，卡1的尺度非常大，那么就会出现卡0始终在等卡1，于是就出现了虽然有的尺度变小了，但是整体的训练速度并没有变快的现象（木桶效应）。解决这个问题的思路就是**尽量把负载拉均衡一些**。
6. 多卡的小batch模拟大batch（梯度累积）的坑。假设我们在单卡下只能塞下batchsize = 2，那么为了模拟一个batchsize = 8的效果，通常的做法是forward 4次，不清理梯度，backward一次（当然从BN的角度出发和单纯的batchsize=8肯定还是不一样的）。但是在多卡下，因为调用loss.backward的时候会做grad的同步，所以说前三次调用backward的时候需要加[ddp.no_sync](https://pytorch.org/docs/stable/generated/torch.nn.parallel.DistributedDataParallel.html?highlight=no_sync#torch.nn.parallel.DistributedDataParallel.no_sync)的context manager（不加的话，第一次bp之后，各个卡上的parameter此时会进行同步），最后一次则不需要加。当然，我看很多仓库并没有这么做，我只能理解他们就是单纯想做梯度累积（BTW，加了ddp.no_sync会使得程序快一些，毕竟加了之后bp过程是无通讯的）。
...
一口气写了这么多条也有点累了，后续有新坑的话我也会继续更新的。毕竟我的目标是工作中不再会有人踩类似的坑 & 炼丹师对深度学习框架有意识，坑不尽，则帖不灭。
最后放一点我个人的小私货吧。不指望每个人在写code的时候能把code本身当作艺术品一样去打磨，但希望大家写代码能有一个最基本的意识：**code本身是给人看的，不是给编译器看的**。糙快猛之后，记得还一下技术债。